...
class AsmDiskOfflineRule(BaseRule):
    ...
    def match(self, file_path: str, content: str) -> RuleResult:
        ...
        for i, line in enumerate(content.splitlines(), 1):
            if re.search(r"OFFLINE", line, re.IGNORECASE) and \
               re.search(r"(disk|ASM)", line, re.IGNORECASE):
                result.matched = True
                result.evidence.append(Evidence(
                    ...
                    detail="ASM 磁盘离线可能导致磁盘组 redundancy 降级。"
                ))
        return result
        ...
        for i, line in enumerate(content.splitlines(), 1):
            if re.search(r"I/O error|IO error|disk error|path error|SCSI error", line, re.IGNORECASE):
                ...
                result.evidence.append(Evidence(
                    ...
                    line_number=i,
                    recommendation=self.recommendation,
                    detail="存储 I/O 错误可能导致数据库性能下降或数据损坏。"
                ))
        return result
