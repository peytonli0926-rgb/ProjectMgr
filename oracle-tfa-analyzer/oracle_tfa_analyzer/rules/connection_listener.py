            ...
            if re.search(r"listener|TNS|LSNR", line, re.IGNORECASE) and \
               re.search(r"(?:error|fail|stop|down|refuse|closed)", line, re.IGNORECASE):
                ...
                result.evidence.append(Evidence(
                    category=self.category, rule_id=self.rule_id, title="监听器异常",
                    severity=self.severity, log_file=file_path,
                    log_snippet=line.strip(), line_number=i,
                    recommendation=self.recommendation, detail="监听器异常会导致数据库无法接受新连接。"
                ))
        return result
...
class ConnectionStormRule(BaseRule):
    ...
    recommendation = "检查应用连接池配置，确认 max_connections 是否合理"

    def match(self, file_path: str, content: str) -> RuleResult:
        result = RuleResult()
        conns = sum(1 for line in content.splitlines() if re.search(r"connect|established|accept", line, re.IGNORECASE))
        if conns > 100:
            result.matched = True
            result.evidence.append(Evidence(
                category=self.category, rule_id=self.rule_id,
                title=f"文件中共 {conns} 个连接记录", severity=self.severity,
                log_file=file_path, log_snippet=f"总连接记录数: {conns}",
                recommendation=self.recommendation, detail=f"listener 日志中发现 {conns} 条连接记录，可能存在连接风暴。"
            ))
        return result
    ...
    def match(self, file_path: str, content: str) -> RuleResult:
        ...
        for i, line in enumerate(content.splitlines(), 1):
            if re.search(r"ORA-12170|TNS-12170|TNS-12535", line, re.IGNORECASE):
                result.matched = True
                result.evidence.append(Evidence(
                    category=self.category, rule_id=self.rule_id, title="TNS 连接超时",
                    severity=self.severity, log_file=file_path,
                    log_snippet=line.strip(), line_number=i,
                    recommendation=self.recommendation, detail="TNS 连接超时通常由网络延迟、防火墙或 listener 负载过高引起。"
                ))
        return result
