...
class TopSqlByElapsedRule(BaseRule):
    ...
    recommendation = "分析执行计划，检查索引、统计信息、并行度"
                ...
                try:
                    ...
                    if val > 60:
                        ...
                        result.evidence.append(Evidence(
                            ...
                            title=f"SQL 耗时 {num_str} {unit}", severity=self.severity,
                            log_file=file_path, log_snippet=line.strip(), line_number=i,
                            recommendation=self.recommendation, detail=f"SQL 执行时间 {val:.1f}s，超过 60s 阈值。"
                        ))
                        break
                except ValueError:
                    continue
...
class WaitEventEnqueueRule(BaseRule):
    ...
    recommendation = "检查锁定对象、长时间未提交事务、应用锁冲突"
    ...
    def match(self, file_path: str, content: str) -> RuleResult:
        ...
        for i, line in enumerate(content.splitlines(), 1):
            if re.search(r"enq:|enqueue|TX|TM|HW|UL|buffer busy|row lock", line, re.IGNORECASE):
                result.matched = True
                result.evidence.append(Evidence(
                    category=self.category, rule_id=self.rule_id, title="Enqueue/锁等待事件",
                    severity=self.severity, log_file=file_path,
                    log_snippet=line.strip(), line_number=i,
                    recommendation=self.recommendation, detail=f"检测到锁争用等待事件: {line.strip()[:80]}"
                ))
        return result
        ...
        for i, line in enumerate(content.splitlines(), 1):
            if re.search(r"temp|temporary|sort", line, re.IGNORECASE) and \
               re.search(r"(?:used|usage|alloc|extend|full)", line, re.IGNORECASE):
                result.matched = True
                result.evidence.append(Evidence(
                    category=self.category, rule_id=self.rule_id, title="临时表空间使用异常",
                    severity=self.severity, log_file=file_path,
                    log_snippet=line.strip(), line_number=i,
                    recommendation=self.recommendation, detail=f"临时表空间异常: {line.strip()[:80]}"
                ))
        return result
