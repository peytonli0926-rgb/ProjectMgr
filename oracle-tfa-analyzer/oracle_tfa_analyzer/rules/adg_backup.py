        ...
        for i, line in enumerate(content.splitlines(), 1):
            if re.search(r"gap|fetching gap|archive gap|log gap|GAP", line):
                ...
                result.evidence.append(Evidence(
                    category=self.category, rule_id=self.rule_id, title="Data Guard 日志缺口",
                    severity=self.severity, log_file=file_path,
                    log_snippet=line.strip(), line_number=i,
                    recommendation=self.recommendation, detail="备库与主库之间存在日志缺口。"
                ))
        return result
            ...
            if m:
                try:
                    ...
                    result.evidence.append(Evidence(
                        ...
                        title=f"DG 同步延迟 {val} 分钟", severity=level,
                        log_file=file_path, log_snippet=line.strip(), line_number=i,
                        recommendation=self.recommendation, detail=f"Data Guard 同步延迟 {val} 分钟。"
                    ))
                except ValueError:
                    pass
        return result
...
class RmanErrorRule(BaseRule):
    ...
    def match(self, file_path: str, content: str) -> RuleResult:
        result = RuleResult()
        for i, line in enumerate(content.splitlines(), 1):
            if re.search(r"RMAN-\d{4,5}|ORA-\d{5}", line) and re.search(r"error|fail|abort", line, re.IGNORECASE):
                result.matched = True
                result.evidence.append(Evidence(
                    category=self.category, rule_id=self.rule_id, title="RMAN 备份错误",
                    severity=self.severity, log_file=file_path,
                    log_snippet=line.strip(), line_number=i,
                    recommendation=self.recommendation, detail=f"RMAN 备份遇到错误: {line.strip()[:80]}"
                ))
        return result
