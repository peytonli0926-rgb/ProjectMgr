        ...
        for i, line in enumerate(content.splitlines(), 1):
            if re.search(r"ORA-07445", line, re.IGNORECASE):
                ...
                result.evidence.append(Evidence(
                    ...
                    log_snippet=line.strip(), line_number=i,
                    recommendation=self.recommendation,
                    detail="ORA-07445 表示收到了 OS 异常信号（如 SIGSEGV），需分析 call stack。"
                ))
        return result
        ...
        for i, line in enumerate(content.splitlines(), 1):
            if re.search(r"Shutting down instance|terminating instance|instance terminated|abnormal termination", line, re.IGNORECASE):
                ...
                result.evidence.append(Evidence(
                    ...
                    title="实例异常终止", severity=self.severity,
            ...
            if re.search(r"Starting ORACLE instance|startup mounted|startup open", line, re.IGNORECASE):
                ...
                result.evidence.append(Evidence(
                    category=self.category, rule_id=self.rule_id,
                    title=f"实例启动: {line.strip()[:60]}", severity=self.severity,
                    log_file=file_path, log_snippet=line.strip(), line_number=i,
                    recommendation="确认重启是否为计划内操作",
    ...
    def match(self, file_path: str, content: str) -> RuleResult:
        ...
        for i, line in enumerate(content.splitlines(), 1):
            ...
            if m and m.group(1) not in self._excluded:
                ...
                result.evidence.append(Evidence(
                    category=self.category, rule_id=self.rule_id,
                    title=f"ORA-{err_code} 错误", severity=self.severity,
                    log_file=file_path, log_snippet=line.strip(), line_number=i,
                    recommendation=self.recommendation,
